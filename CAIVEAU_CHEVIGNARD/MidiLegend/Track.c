...
void render_particle(Track* self)
{
    ...
    for (int i = 0; i < MAX_PARTICLE_COUNT; i++)//parcours du tableau 
    {
        ...
        dst2.h = dst2.w;
        dst2.x = (int)self->particule[i].xposition;//position équivalente au milieu de la note préssé
...
void create_particle(Track* self, int keyID)
{
    ...
    for (int i = 0; i < MAX_PARTICLE_COUNT; i++)//parcours du tableau
    {
        if (self->particule[i].duration <= 0)//recherche de la première particule inactive
        {
            ...
            self->particule[i].xposition = 10 + (580 - ((self->keyCount - 1) * 100 + 40)) / 2 + 360.0f + keyID * 100;//position de la note préssé
            self->particule[i].yposition = (float)(validationRelPos * 685) + 25;//position de la note préssé
            self->particule[i].yspeed = (float)(rand() % 150) - 125;//vitesse initiale aléatoire entre -125 et 25
            self->particule[i].xspeed = (float)(rand() % 100) - 50;// vitesse initiale aléatoire entre -50 et 50
